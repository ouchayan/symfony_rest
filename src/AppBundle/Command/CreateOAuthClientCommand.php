...
class CreateOAuthClientCommand extends ContainerAwareCommand {
    ...
    protected function configure() {
        $this
            ...
            ->addArgument(
                'name', InputArgument::REQUIRED, 'Client Name?'
            )
            ->addArgument(
                'redirectUri', InputArgument::REQUIRED, 'Redirect URI?'
            )
            ->addArgument(
                'grantType', InputArgument::REQUIRED, 'Grant Type?'
            );
    }
    ...
    protected function execute(InputInterface $input, OutputInterface $output) {
        ...
        $output->writeln(sprintf("<info>The client <comment>%s</comment> was created with <comment>%s</comment> as public id and <comment>%s</comment> as secret</info>", $client->getName(), $client->getPublicId(), $client->getSecret()));

        if ($grantType == "password") {
            $customers = $container->get('doctrine')->getRepository('AppBundle:User')->findAll();
            foreach ($customers as $customer) {
                ...
                $output->writeln(sprintf("<info>User <comment>%s</comment> linked to client <comment>%s</comment></info>", $customer->getId(), $client->getName()));
            }
        }
    }
